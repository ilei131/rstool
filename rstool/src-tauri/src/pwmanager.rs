...
use tauri::State;
use crate::pwmanager::cryptography::EncryptedBlob;
use crate::pwmanager::database::CredentialsDatabase;
use crate::pwmanager::error::Error;

pub static APP_FOLDER: Lazy<PathBuf> = Lazy::new(|| {
    if cfg!(target_os = "windows") {
      let appdata = std::env::var("APPDATA").expect("$APPDATA not set!");
      [&appdata, "rstool"].iter().collect()
    } else {
      let home = std::env::var("HOME").expect("$HOME not set!");
      [&home, ".config", "rstool"].iter().collect()
    }
  });
...
#[tauri::command]
pub fn login(username: String, password: String, session: State<'_, Mutex<Option<UserSession>>>) -> Result<(), Error> {
...
#[tauri::command]
pub fn create_account(username: String, password: String, session: State<'_, Mutex<Option<UserSession>>>) -> Result<(), Error> {
